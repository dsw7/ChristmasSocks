...
self.buffer_size = socks_client.configs['client'].getint('tcp_buffer_size') - 1
...
def test_echo_max_size_minus_one_byte_string(socks_server_background, socks_client) -> None:
    string = utils.generate_random_string(num_strings=1, len_strings=self.buffer_size)
...
def test_echo_max_size_plus_five_bytes_string(socks_server_background, socks_client) -> None:
    string = utils.generate_random_string(num_strings=1, len_strings=self.buffer_size + 5)
